...
function NotificationCard ({notification, key, deleteNotificationHandler}) {
    ...
    return (
        <>
        {
            notification.type === 'post' || notification.type === 'share' || notification.type === 'tag' && 
            <div className='w-full h-auto flex flex-row items-center'>
                <div className='w-[calc(100%-50px)] h-auto flex flex-row items-center px-6 py-2 bg-[#eaeaea] dark:bg-[#232323] rounded-2xl cursor-pointer duration-200 ease-in-out'>
                    ...
                    <div className='w-auto h-auto flex flex-col gap-1 pl-6'>
                        <div><span className='kanit text-[#232323] dark:text-white'>{notification.from.fullname}</span>
                        <span className='text-[#555555] dark:text-[#bcbcbc] text-[0.9rem] radio'>
                            {
                                notification.type === 'post' ?
                                ' posted something on your feed' : 
                                notification.type === 'share' ?
                                '  shared a post on your feed ' : 
                ...
                <div className={`w-[50px] h-auto flex items-center justify-center relative`}>
                    ...
                    <div 
                        className={`${deleteNotification ? 'opacity-100 w-[190px] h-auto' : 'opacity-0 w-0 h-0'} absolute right-[100%] bg-white hover:bg-[#f5f5f5] duration-500 ease-in-out px-4 py-2 shadow-[0_0_10px_0_rgba(0,0,0,0.1)] flex flex-row items-center justify-center rounded-lg cursor-pointer`}
                        onClick={() => {
...

        {
            ...
            <div className='w-full h-auto flex flex-row items-center justify-between'>
                ...
                <div className={`w-[50px] h-auto flex items-center justify-center relative`}>
                    ...
                    <div 
                        className={`${deleteNotification ? 'w-[190px] h-auto scale-100' : 'w-0 h-0 scale-0'} absolute right-[100%]  bg-white hover:bg-[#f5f5f5] duration-200 ease-in-out px-4 py-2 shadow-[0_0_10px_0_rgba(0,0,0,0.1)] flex flex-row items-center justify-center rounded-lg cursor-pointer`}
                        onClick={() => {
                            deleteNotificationHandler(notification._id)
                        }}
                    >
                        <svg className={` ${deleteNotification ? 'scale-100 duration-300' : 'scale-0 duration-100'}`} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-trash-2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>
                        <div className={` ${deleteNotification ? 'scale-100 duration-300' : 'scale-0 duration-100'}  ease-in-out pl-2 radio text-[0.9rem] inline-block`}>Delete Notification</div>
                    </div>
                </div>
            </div>
        }
            ...
            <div className='w-full h-auto flex flex-row items-center justify-between'>
                <div className='w-[calc(100%-50px)] h-auto flex flex-row items-center justify-between px-6 py-2 bg-[#eaeaea] dark:bg-[#232323] rounded-2xl cursor-pointer duration-200 ease-in-out'>
                    <div className='w-auto h-auto flex flex-row items-center'>
                        <img src={notification.from.profilePic} alt="" className='w-[60px] h-[60px] rounded-full'/>
                        <div className='w-auto h-auto flex flex-col gap-1 pl-6'>
                            <div><span className='kanit text-[#232323] dark:text-white'>{notification.from.fullname}</span><span className='text-[#555555] dark:text-[#999999] text-[0.9rem] radio'> liked your post</span></div>
                            <div className='text-[0.8rem] radio text-[#777777]'>{formattedTime}</div>
                        </div>
                    </div>
                </div>
