...
function PostCreation ({ postCreation, setPostCreation }) {
  ...
  return (
    <div className={`${postCreation ? 'translate-x-0 blur-0 opacity-100 sclae-100' : 'translate-x-[100%] blur-xl opacity-0 scale-0'} duration-700 ease-in-out w-full fixed top-[70px] z-20`}>
        <div className='w-full h-[calc(100vh-70px)] flex items-center justify-center'>
            <div className='w-auto min-w-[330px] min-h-[230px] bg-white/70 flex flex-col gap-6 items-center justify-center rounded-3xl backdrop-blur-2xl'>
                <NavLink to={'/post/story'} 
                onClick={() => setPostCreation(false)}
                className={({isActive}) => {
                    isActive ? setActive1(true) : setActive1(false)
                    return `${isActive ? ' bg-[#232323] text-blue-500' : ' text-[#232323]'} flex flex-row gap-6 px-8 py-3 rounded-2xl max-w-[300px]`
                }}>
