...
function CommentBox ({ post, commentBoxOpen, setCommentBoxOpen, likeCount, setLikeCount, commentCount, setCommentCount, likedPost, setLikedPost, handleLike }) {
	...
	return (
		<div className='w-full h-full relative'>
			...
			<div className='text-[1.3rem] text-[#555555] exo px-10 py-3 w-full dark:text-white absolute top-0 left-0 bg-white dark:bg-[#232323] overflow-hidden z-30'>
				{post?.user?.fullname.split(' ')[0]}'s post
			</div>

			<div 
				className='absolute top-2 right-2 text-[0.9rem] text-white dark:text-black font-light bg-[#232323] dark:bg-white hover:scale-110 duration-300 ease-in-out px-5 py-1.5 rounded-xl kanit cursor-pointer z-30' 
				onClick={() => setCommentBoxOpen(!commentBoxOpen)}
			>
				BACK
			</div>

			<div className='w-full h-full flex flex-col items-center overflow-hidden overflow-y-auto' id='menuScroll'>

				<div className='w-[80%] flex flex-row h-auto mt-[70px] py-2 rounded-xl bg-white dark:bg-[#151515]'>
					...
					<div className='flex flex-col justify-center'>
						<Link className='text-black dark:text-white kanit text-[1.1rem]'>{post.user.fullname}</Link>
						<small className='text-[#777777] dark:text-[#aaaaaa] radio'>{formattedTime}</small>
					</div>
				</div>

				{
					post?.images?.length > 0 ? 
					<div className='w-[80%] mx-auto pt-2 pl-3 kanit text-[#232323]'>
						{post?.text}
					</div> : 
					<div className='w-[80%] mx-auto p-6 kanit text-[#232323] text-[1.2rem] bg-[#efefef] dark:bg-[#111111] dark:text-white rounded-2xl mt-8'>
						{post?.text}
					</div>
				}
				...
				<div className='w-[50%] mx-auto h-auto flex flex-row justify-between items-center bg-[#efefef] dark:bg-[#434343] my-2 rounded-xl opacity-60 dark:opacity-90 px-10'>
                    <div className='flex flex-row items-center gap-1 px-4 py-1.5'>
                        <div className='text-[0.8rem] kanit dark:text-white'>
							{likeCount}
						</div>
                    ...
                    <div className='flex flex-row items-center gap-1 px-4 py-1.5'>
                        <div className='text-[0.8rem] kanit dark:text-white'>{commentCount}</div>
				...
				<div className='h-auto flex flex-row justify-center items-center bg-[#f7f7f7] dark:bg-[#454545] my-2 rounded-xl w-[50%] mx-auto gap-10'>
					<div 
                        onClick={handleLike} className={`${likedPost ? 'bg-blue-100 dark:bg-[#263038]' : 'bg-[#eeeeee] dark:bg-[#333333]'} flex flex-row gap-2 px-8 py-2.5 rounded-xl cursor-pointer hover:scale-110 duration-200 ease-in-out items-center`}>
                        {
                            likedPost ?
                            <>
				...
				<div className='w-[80%] md:w-[70%] h-auto rounded-xl mx-auto px-6 py-4 mb-[80px] bg-[#f4f4f4] dark:bg-[#111111] z-20'>
				{
					...
					<>
					{
						...
						<div className='text-[0.9rem] kanit font-light text-[#555555]'>
							No Comments Yet
						</div>
					</>
				}
			...
			<div className='absolute w-full bottom-0 flex flex-row gap-4 justify-center items-center z-20 bg-white dark:bg-[#232323] py-2'>
				<img src={user.profilePic} alt=""  className='w-[40px] h-[40px] object-cover object-center rounded-full ring-2 ring-blue-600'/>
				<div className='bg-[#efefef] dark:bg-[#111111] w-[70%] flex flex-row rounded-xl items-center justify-between px-2'>
					<input 
						type="text" 
						value={comment}
						...
						/>
					<div className='w-[35px] h-[35px] rounded-xl hover:bg-[#dddddd] dark:hover:bg-[#232323] flex items-center justify-center cursor-pointer duration-200 ease-in-out'
						onClick={() => {
