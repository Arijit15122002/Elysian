			...
			<></> : <>
				<div className='w-full h-full flex flex-col items-center'>
					...
					<div className='w-full h-[57%] my-4 flex items-start'>
						<div className='w-full lg:w-[90%] xl:w-[87%] mx-0 xl:mx-4 h-full bg-white dark:bg-[#232323] rounded-3xl overflow-hidden' style={{
							boxShadow: '0 0 10px 0 rgba(0, 0, 0, 0.1)'
						}}>
							<div className='flex flex-row items-center h-[70px] w-full justify-between'>
								<div className='px-6 kanit text-[1.1rem] lg:text-[1.2rem] text-[#777777] dark:text-white dark:font-light'>Suggestions</div>
								<Link to={'/explore'} className=' flex flex-row gap-0.5 items-center text-blue-500 px-4 cursor-pointer'>
					...
					<div className='w-full h-[43%] flex flex-col items-start justify-items-start'>
						<div className='w-full lg:w-[90%] xl:w-[87%] mx-0 xl:mx-4 h-[90%] bg-white dark:bg-gradient-to-tr from-[#232323] to-[#151515] rounded-3xl overflow-hidden flex flex-col items-center' style={{
							boxShadow: '0 0 10px 0 rgba(0, 0, 0, 0.1)',
						}}>
							<div className='w-full h-[60px] px-6 py-2 flex flex-row gap-2 items-center justify-between relative '>
								<div className='w-auto flex flex-row gap-2 items-center'>
									...
									<div className='text-[#555555] dark:text-white kanit dark:font-light text-[1.1rem] hidden xl:block'>
										Notifications
									</div>
									<div className={`${ unreadNotifications.length > 0 ? 'block' : 'hidden' } absolute top-[18px] left-[35px]`}>
										<span class="relative flex h-[8px] w-[8px]">
											...
											<span class="relative inline-flex rounded-full h-[8px] w-[8px] bg-blue-600"></span>
										</span>
									</div>
								</div>
								<Link to={'/notifications'} className='flex flex-row gap-0.5 items-center text-blue-500 cursor-pointer'>
									<div className='radio text-[0.75rem]'>SEE ALL</div>
									<svg className='-mt-[1px]' xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-right"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
								</Link>
							</div>
							<div className='h-[calc(100%-70px)] w-[90%] rounded-2xl flex flex-col items-center gap-3 flex-shrink-0 overflow-y-auto' id='menuScroll'>
							{
								unreadNotifications.map((notification, index) => (
								<>
									<FeedNotificationCard notification={notification} key={index} />
								</>
								))
							}
							</div>
